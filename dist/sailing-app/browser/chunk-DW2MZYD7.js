...
var HVAR_REGION_CODES = ["HR805", "HR806", "HR008"];
var HVAR_REGION_NAMES = ["middle dalmatia", "south dalmatia", "split", "hvar"];
var SEVERITY_COLORS = {
  Minor: "#4caf50",
  Moderate: "#ff9800",
  Severe: "#f44336",
...
var WeatherService = class _WeatherService {
  ...
  RADAR_BASE = "https://vrijeme.hr/radari/debeljak_1300.png";
  // Proxied via /api/meteoalarm/ → feeds.meteoalarm.org (CORS workaround)
  WARNINGS_URL = "/api/meteoalarm/feeds-croatia";
  radarUrl = signal(this.buildRadarUrl());
  radarUpdatedAt = signal(/* @__PURE__ */ new Date());
  allWarnings = signal([]);
  hvarOnly = signal(true);
  warnings = computed(() => {
    const all = this.allWarnings();
    if (!this.hvarOnly())
      return all;
    return all.filter((w) => w.regionCodes.some((c) => HVAR_REGION_CODES.includes(c)) || w.regions.some((r) => HVAR_REGION_NAMES.some((n) => r.toLowerCase().includes(n))));
  });
  warningsLoading = signal(false);
  warningsError = signal(null);
  warningsUpdatedAt = signal(null);
  startRadarPolling(intervalMs = 3e5) {
    interval(intervalMs).pipe(takeUntilDestroyed(this.destroyRef)).subscribe(() => this.refreshRadar());
  }
  startWarningsPolling(intervalMs = 15 * 6e4) {
    interval(intervalMs).pipe(takeUntilDestroyed(this.destroyRef)).subscribe(() => this.loadWarnings());
  }
  refreshRadar() {
    this.radarUrl.set(this.buildRadarUrl());
    this.radarUpdatedAt.set(/* @__PURE__ */ new Date());
  }
  loadWarnings() {
    this.warningsLoading.set(true);
    this.warningsError.set(null);
    this.http.get(this.WARNINGS_URL).pipe(map((response) => this.parseWarnings(response)), catchError((err) => {
      ...
      next: (w) => {
        ...
        this.warningsUpdatedAt.set(/* @__PURE__ */ new Date());
      },
      error: () => {
  ...
  parseWarnings(response) {
    ...
    return response.warnings.map((w) => {
      const alert = w.alert;
      const info = alert.info.find((i) => i.language === "en-GB") ?? alert.info.find((i) => i.language?.startsWith("en")) ?? alert.info[0];
      if (!info)
        return null;
      ...
      const regionCodes = info.area.flatMap((a) => (a.geocode ?? []).filter((g) => g.valueName === "EMMA_ID").map((g) => g.value));
      return {
        id: alert.identifier,
        country: "HR",
...
//# sourceMappingURL=chunk-DW2MZYD7.js.map
